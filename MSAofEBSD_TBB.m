...
for n=1:NumPCA
    subplot(2,5,n);
    imagesc(reshape(coeff(:,n),[PatSizeW,PatSizeH])); axis image; axis xy; axis tight; colormap('gray'); axis off;
end
figure;
for n=1:NumPCA
    subplot(4,5,n);
    imagesc(reshape(score(:,n),[Data_InputMap.ypts Data_InputMap.xpts])); axis image; axis ij; axis tight; axis off;
end

%rotate the factors
[coeffVM, RotVM] = rotatefactors(coeff(:,1:NumPCA)+mean(mean(coeff)),'Method','varimax', 'Maxit', 5000, 'Reltol', 0.5*sqrt(eps));
coeffVM=coeffVM-mean(mean(coeff));
...
for n=1:NumPCA
    subplot(4,5,n);
    imagesc(reshape(scoreVM(:,n),[Data_InputMap.ypts Data_InputMap.xpts])); axis image; axis ij; axis tight; axis off;
end
...
figure;
subplot(1,2,1); imagesc(scoremap_v); axis image; axis xy; axis ij;
